...
def download():
    try:
        t = paramiko.Transport(('192.168.1.115', 22))
        t.connect(username='root', password='123456')
        sftp = paramiko.SFTPClient.from_transport(t)

        src = '/home/yangtianfeng/Desktop/apache-tomcat-7.0.88/webapps/darams.war'
        des = '/home/darams.war'
        sftp.get(src, des)
        t.close()
        return True
...
def copy(remote_default_path,remote_purpose_path,port,url):
    ...
    try:
        if a[0] is True:
            ...
            # remote_size = os.path.getsize('F:\\Users\\Administrator\\darams.war')
            if remote_size == a[1]:
                logger.debug('上传成功')
                print('上传成功')
