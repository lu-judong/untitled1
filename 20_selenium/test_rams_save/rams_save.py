...
from config.config import path_dir,url,username,password,ramsSave_config
...
class test_technology:
    ...
    def tech(self,url,username,password,modelName,value,remarks,select,start,end,average,car,fault,select_fault,fault1,time_sleep,wait_time):
        ...
        self.log_file_out('-----RAMS指标建模-----')

        for i in ramsSave_config:
            try:
...
test_technology().tech(url,username,password,'1.16测试',0,'','时间','2017-02-03','2017-10-02',2,car,fault_pattern,'交集',fault1,time_sleep,wait_time)