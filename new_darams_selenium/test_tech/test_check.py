...
class tech:
    ...
    # 连接数据库提取数据
    def select_value(self,modelName,count):
        sql_no_obj = \
            '''
             SELECT
                d.start_date,
            ...
            WHERE 
                m.model_type = 'TECHNICAL_OPTIMIZATION_MODEL'
                and m.model_name = '{}'
            '''.format(modelName)
        ...
        if input_value[0][0] is not None:
            ...
            for i in range(0,len(L)-1):
                ...
                # 累计里程
                dist_basic_total_km = self.mtbf_km(L[i].strftime("%Y-%m-%d"), L[i + 1].strftime("%Y-%m-%d"),tuple_trainno)
                if '基本故障' in a[0].keys():
                    fcnt_total = a[0]['基本故障']
            ...
            b = self.faultNum(sql_start, sql_end, tuple_trainno, sql_fault, sql_pattern, late_from, late_end,
                              late_type)
            dist_basic_total_km = self.mtbf_km(L[0].strftime("%Y-%m-%d"), L[-1].strftime("%Y-%m-%d"), tuple_trainno)
            if '基本故障' in b[2].keys():
                fault_d = b[2]['基本故障']
                for i in fault_d:
                    if dist_basic_total_km == 0:
