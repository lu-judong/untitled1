...
import time
from config.config import path_dir,singelmodel_config
...
class Singel:
    ...
    def picture(self,url,username,password):
        ...
        js = 'var aa = echarts.getInstanceByDom($("#maxMin")[0]);' \
             ...
             'bb = param.name;'\
             'jp.openDialogView ("", "/darams/a/mould/confModel/skipTo2?XValue=" + param.name +                          "&chartCode=CHART_MIN_MILLION_KILOMETERS_FAILURE_RATE" + "&YValue=" + param.value + "&chartName=" + "最大百万公里故障率" + "&lineType=" + "关联故障" + "&confModelId=2bad05e26f0f4effa84e49a880165d9d000&chartType=index", "100%", "100%");'\
            'return bb'
