...
from interfaceTest.testInput.test_nhppEvaluate_input.test_4_nhppEvaluate_charts import *


@ddt.ddt
class test_nhpp(unittest.TestCase):
    ...
    @ddt.file_data('{}/testInput/test_nhppEvaluate_input/test_2_nhppEvaluate.json'.format(path_dir))
    def test_2_nhppEvaluate(self,modelId):
        """nhpp计算接口"""  # 说明测试用例的标题
        ...
        if len(globals()["Authorization"]) == 0:
            ...
            write_sheet(path, 'nhpp计算接口', '','','用户未登录','')
            self.assertEqual(3, 4)
        else:
            try:
                driver = self.driver
                driver.refresh()
                time.sleep(2)
                Method(driver).click('xpath', '//span[contains(text(),"nhpp-model-controller")]')
                time.sleep(1)
                ...
                if evaluate_send.status_code == 200:
                    ...
                    if evaluate_response['success'] is True:
                        ...
                        if 'result'in evaluate_response.keys():
                            ...
                            write_sheet(path, 'nhpp计算接口', url, params, json.dumps(evaluate_response, ensure_ascii=False), json.dumps(result))
                        else:
                            write_sheet(path, 'nhpp计算接口', url, params, json.dumps(evaluate_response, ensure_ascii=False),'')

                        self.assertEqual(3, 4)
                else:
                    write_sheet(path, 'nhpp计算接口', url, params, 'nhpp计算接口返回'.format(evaluate_send.status_code),'')
    ...
    @ddt.file_data(
        '{}/testInput/test_nhppEvaluate_input/test_4_nhppEvaluate_charts.json'.format(path_dir))
    def test_3_nhppEvaluate_charts1(self, modelId):
        """nhpp图表1接口"""
        path = self.path
        if len(globals()["Authorization"]) == 0:
            write_sheet(path, 'nhpp图表1接口', '', '', '用户未登录', '')
            ...
            if charts1_interface_status != 'non-existent':
                try:
                    ...
                    # 判断文件中是否存在
                    file = '{}/testFile/test_nhppEvaluate/test_nhppEvaluate_charts1/test_nhppEvaluate_charts_{}.txt'.format(
                        path_dir, modelId)
                        ...
                        if not sz:
                            ...
                            if dict_fp == dict_ramscharts:
                                ...
                                if 'result' in dict_ramscharts.keys():
                                    ...
                                    write_sheet(path, 'nhpp图表1接口', url, params,
    ...
    @ddt.file_data('{}/testInput/test_nhppEvaluate_input/test_4_nhppEvaluate_charts.json'.format(path_dir))
    def test_4_faultEvaluate_charts2(self, modelId):
        """nhpp图表2接口"""
        path = self.path
        if len(globals()["Authorization"]) == 0:
            write_sheet(path, 'nhpp图表2接口', '', '', '用户未登录', '')
            ...
            if charts2_interface_status != 'non-existent':
                try:
                    ...
                    # 判断文件中是否存在
                    file = '{}/testFile/test_nhppEvaluate/test_nhppEvaluate_charts2/test_nhppEvaluate_charts2_{}.txt'.format(
                        path_dir, modelId)
                        ...
                        if not sz:
                            ...
                            if dict_fp == dict_ramscharts:
                                ...
                                if 'result' in dict_ramscharts.keys():
                                    ...
                                    write_sheet(path, 'nhpp图表2接口', url, params,
                                                json.dumps(dict_ramscharts, ensure_ascii=False),
                                                json.dumps(result, ensure_ascii=False)[:2000])
                                else:
                                    write_sheet(path, 'nhpp图表2接口', url, params,
    ...
    @ddt.file_data(
        '{}/testInput/test_nhppEvaluate_input/test_4_nhppEvaluate_charts.json'.format(path_dir))
    def test_5_faultEvaluate_charts3(self, modelId):
        """nhpp图表3接口"""
        path = self.path
        if len(globals()["Authorization"]) == 0:
            write_sheet(path, 'nhpp图表3接口', '', '', '用户未登录', '')
            ...
            if charts3_interface_status != 'non-existent':
                try:
                    ...
                    # 判断文件中是否存在
                    file = '{}/testFile/test_nhppEvaluate/test_nhppEvaluate_charts3/test_nhppEvaluate_charts3_{}.txt'.format(
                        path_dir, modelId)
                        ...
                        if not sz:
                            ...
                            if dict_fp == dict_ramscharts:
                                ...
                                if 'result' in dict_ramscharts.keys():
                                    ...
                                    write_sheet(path, 'nhpp图表3接口', url, params,
                                                json.dumps(dict_ramscharts, ensure_ascii=False),
                                                json.dumps(result, ensure_ascii=False)[:2000])
                                else:
                                    write_sheet(path, 'nhpp图表3接口', url, params,
    ...
    @ddt.file_data(
        '{}/testInput/test_nhppEvaluate_input/test_4_nhppEvaluate_charts.json'.format(path_dir))
    def test_6_repairSystemEvaluate_charts4(self, modelId):
        """nhpp图表4接口"""
        path = self.path
        if len(globals()["Authorization"]) == 0:
            write_sheet(path, 'nhpp图表4接口', '', '', '用户未登录', '')
            ...
            if charts4_interface_status != 'non-existent':
                try:
                    ...
                    # 判断文件中是否存在
                    file = '{}/testFile/test_nhppEvaluate/test_nhppEvaluate_charts4/test_nhppEvaluate_charts4_{}.txt'.format(
                        path_dir, modelId)
                        ...
                        if not sz:
                            ...
                            if dict_fp == dict_ramscharts:
                                ...
                                if 'result' in dict_ramscharts.keys():
                                    ...
                                    write_sheet(path, 'nhpp图表4接口', url, params,
                                                json.dumps(dict_ramscharts, ensure_ascii=False),
                                                json.dumps(result, ensure_ascii=False)[:2000])
                                else:
                                    write_sheet(path, 'nhpp图表4接口', url, params,
...
if __name__ == '__main__':
    ...
    st.addTest(unittest.makeSuite(test_nhpp))
    # unittest.main()
